...
fun MainScreen(navController: NavController, usersViewModel: UsersViewModel) {
    ...
    val tags = listOf(
        Tag(1, "multiplayer", "Multijugador"),
        Tag(2, "singleplayer", "Un jugador"),
        Tag(3, "co-op", "Cooperativo"),
        Tag(4, "open world", "Mundo abierto"),
        Tag(5, "indie", "Independiente")
    )
        ...
        Column(modifier = Modifier.fillMaxSize()) {
            ...
            if (searchExpanded) {
                Column(modifier = Modifier.padding(8.dp)) {
                    ...
                    OutlinedTextField(
                        value = tagsQuery,
                        onValueChange = { tagsQuery = it },
                        modifier = Modifier.fillMaxWidth(),
                        label = { Text("Buscar por etiquetas") },
                        singleLine = true
                    )
                    Spacer(modifier = Modifier.height(8.dp))
                    Button(
            ...
            if (isLoading) {
                ...
                Column(
                    ...
                    LazyColumn(
                        modifier = Modifier
                            .fillMaxWidth()
                            .fillMaxHeight((if (searchExpanded) 0.84f else 0.90f))
                            .padding(6.dp)
                    ) {
                        items(games) { game ->
